...
const unitOne = {
  ...
  estimatedMinutes: 8,
  timeAllocation: { theory: 5, cases: 2, practice: 1 },
  theory: [
    t(
...
const unitFive = {
  ...
  estimatedMinutes: 9,
  timeAllocation: { theory: 3, cases: 3, practice: 3 },
  theory: [
    t(
...
const unitSix = {
  ...
  estimatedMinutes: 9,
  timeAllocation: { theory: 3, cases: 4, practice: 2 },
  theory: [
    t(
  ...
  interviewAnswers: [
    {
      ...
      short: t(
        `Parto da una perdita operativa misurabile, poi osservo al gemba il processo reale e la decisione da migliorare. Confronto i candidati per valore, frequenza, stabilità, dati, integrazione, rischio, giudizio umano, adozione e tempo al valore. Applico anche gate non compensabili, perché un punteggio medio alto non giustifica una decisione irreversibile o regolamentata. Scelgo il più piccolo MVP end-to-end controllato, con KPI, guardrail, responsabilità umana e gate di scala.`,
      ...
      long: t(
        `Decido sulla base di evidenze e rischio, non partendo da un elenco di tecnologie. Definisco prima la perdita operativa, la baseline e la decisione che deve migliorare. Al gemba mappo il flusso reale, le eccezioni, le fonti dati, i passaggi di responsabilità e i ruoli che agiscono. Semplifico il lavoro instabile o non necessario prima di automatizzarlo. Confronto poi i candidati con una scorecard pesata che copre valore, frequenza e sforzo, stabilità, data readiness, fattibilità di integrazione, rischio e reversibilità, impatto regolatorio, giudizio umano, adozione e tempo al valore. Ogni valutazione richiede evidenza e livello di confidenza. Il punteggio struttura il confronto, mentre i gate impediscono che una media nasconda un rischio inaccettabile. Per esempio, respingerei il rilascio autonomo del prodotto perché il giudizio qualificato e il controllo Quality sono essenziali. Nel portafoglio svolto, la creazione controllata di work order è forte perché frequente, rule-based, reversibile e collegata al downtime. L'MVP valida il contesto evento, prepara una bozza completa, richiede conferma del tecnico, autorizza la scrittura tramite l'applicazione, registra l'esito e mantiene un fallback manuale. Misuro tempo evento-ordine e completezza, poi verifico se migliorano il downtime senza aumentare falsi trigger, backlog, scarto o eventi qualità. Scalo soltanto quando valore, controlli, adozione, ownership e supporto sono dimostrati.`,
        `I decide through evidence and risk, not through a list of technologies. First, I define the operational loss, its baseline, and the decision that needs to improve. At the gemba I map the real workflow, exceptions, data sources, handoffs, and responsible roles. I simplify unstable or unnecessary work before considering automation. I then compare candidates with a weighted scorecard covering business value, frequency and effort, process stability, data readiness, integration feasibility, risk and reversibility, regulatory impact, required human judgment, adoption complexity, and time to value. Each rating needs evidence and a confidence level. The score supports comparison, while hard gates prevent an average from hiding unacceptable risk. For example, I would reject autonomous product release because qualified human judgment and formal quality control are essential. In the worked portfolio, controlled maintenance work-order creation scores strongly because it is frequent, rule-based, reversible, and connected to a downtime loss. The MVP would validate event context, propose a complete draft, require technician confirmation, authorize the write through the application, log the outcome, and retain a manual fallback. I would measure event-to-order time and data completeness, then test whether they improve target downtime without increasing false triggers, backlog, scrap, or quality events. I scale only if value, controls, adoption, ownership, and support are proven.`
      ),
      followUps: [
        t('Quale hard gate potrebbe fermare il candidato con il punteggio più alto?', 'Which hard gate could stop the highest-scoring candidate?'),
        t('Quale evidenza useresti per autorizzare la scala?', 'What evidence would you use to authorize scaling?')
      ]
    },
    {
      prompt: t('Puoi descrivere un esempio di trasformazione digitale applicata alla manifattura?', 'Can you give me an example of applying digital transformation in manufacturing?'),
      short: t(
        `In un caso ipotetico di manifattura regolamentata, una linea perde 1.260 minuti in quattro settimane e i work order arrivano tardi o incompleti. Piloterei la creazione assistita per una causa ricorrente, con validazione e conferma del tecnico. Il target è ridurre il tempo di richiesta da 18 a meno di 5 minuti e il downtime della causa di almeno il 15 per cento, senza aumentare scarto, deviazioni, falsi trigger o backlog.`,
        `In a hypothetical regulated-manufacturing case, a line lost 1,260 minutes in four weeks and work orders were delayed or incomplete. I would pilot assisted work-order creation for one recurring cause, with validation and technician confirmation. The target is to cut request time from 18 to below 5 minutes and reduce cause-related downtime by at least 15 percent without increasing scrap, deviations, false triggers, or backlog.`
      ),
      long: t(
        `Un esempio pratico è un caso realistico ma ipotetico su una linea regolamentata ad alta velocità. La baseline è 1.260 minuti di downtime non pianificato su 30.000 minuti, cioè 4,2 per cento. Una causa ricorrente vale 480 minuti e il gemba mostra 18 minuti mediani tra evento e richiesta completa. Invece di passare subito alla predictive maintenance, verificherei il collo di bottiglia informativo osservato. L'MVP copre una linea e una famiglia causa. Riceve un evento approvato in sola lettura, aggiunge contesto asset e produzione, valida campi obbligatori e duplicati e presenta una bozza di work order. L'operatore conferma il contesto, il tecnico la priorità e un'applicazione autorizzata crea l'ordine registrando ogni passaggio. Non scrive sul controllo e non prende decisioni sul prodotto; il fallback manuale resta disponibile. Inizio in shadow mode. I target sono meno di cinque minuti evento-richiesta, almeno 95 per cento di completezza e almeno 15 per cento di riduzione della causa. I guardrail coprono falsi trigger, backlog, scarto di riavvio e deviazioni. Se un periodo comparabile passa da 480 a 396 minuti con guardrail rispettati, la riduzione del 17,5 per cento giustifica un'estensione controllata, non un rollout globale. È trasformazione perché dati, workflow, ruoli, controlli e apprendimento cambiano insieme attorno a un risultato misurabile.`,
      ...
      followUps: [
        t('Come dimostreresti che il miglioramento deriva dal workflow?', 'How would you show that the improvement came from the workflow?'),
        t('Quale guardrail fermerebbe il pilota?', 'Which guardrail would stop the pilot?')
      ]
    },
    {
      prompt: t('Come distingui un progetto tecnologico da una trasformazione?', 'How do you distinguish a technology project from transformation?'),
      short: t(
        `Un progetto tecnologico consegna una capacità, come sensori, software o dashboard. La trasformazione cambia il modo in cui le persone prendono e governano decisioni, con outcome misurabili e ownership sostenibile. Cerco un legame causale dalla perdita operativa al cambiamento di workflow, ruoli, controlli, KPI, adozione e feedback. Il go-live è un output; la trasformazione è il miglioramento duraturo del sistema socio-tecnico.`,
      ...
      long: t(
        `Li distinguo osservando l'unità di cambiamento e l'evidenza di valore. Un progetto tecnologico può consegnare con successo un output: un sensore connesso, una piattaforma cloud o un modello AI. Può essere necessario e utile, ma non basta per parlare di trasformazione. La trasformazione parte da una perdita definita e ridisegna l'intero ciclo decisionale. Cambia come i dati vengono creati e interpretati, come le persone agiscono, chi risponde del risultato, quali controlli si applicano, come funzionano eccezioni e fallback e come l'organizzazione apprende dagli outcome. Separo output, outcome e impatto. Una dashboard è output; una review quotidiana cross-functional con owner e azioni è outcome; una riduzione sostenuta del downtime senza danneggiare qualità o sicurezza è impatto. Chiedo anche se la capacità continua dopo l'uscita del team: esistono process owner, product ownership, data stewardship, supporto, change control, feedback utenti e decisione di scala? Se mancano, l'iniziativa può restare uno strumento locale utile. La distinzione non dipende dalla complessità. Un workflow deterministico semplice può abilitare trasformazione se cambia l'operating model attorno a una decisione misurabile. Al contrario, una demo AI avanzata può non trasformare nulla se non viene adottata o non opera nei vincoli reali. La tecnologia è un abilitatore; la trasformazione è un cambiamento durevole e governato del sistema socio-tecnico.`,
      ...
      followUps: [
        t('Quale outcome dimostrerebbe un cambiamento durevole?', 'Which outcome would demonstrate durable change?'),
        t('Chi deve possedere la capacità dopo il go-live?', 'Who must own the capability after go-live?')
      ]
    }
  ],
  finalQuiz: [
